...
class BybitExchange(BaseExchange):
    """Bybit exchange client using ccxt-pro."""
    
    def __init__(self, api_key: str, api_secret: str, testnet: bool = False):
        super().__init__(api_key, api_secret, testnet)
        self.client = ccxtpro.bybit({
            'apiKey': api_key,
            'secret': api_secret,
            'enableRateLimit': True,
            'options': {
                'defaultType': 'swap',  # Use USDT perpetuals by default
                'test': testnet
            }
        })
    ...
    async def fetch_balance(self, currency: Optional[str] = None) -> Dict[str, Balance]:
        ...
        try:
            ...
            for curr, data in balance_data.get('info', {}).get('result', {}).get('list', [{}])[0].get('coin', []).items():
                if isinstance(data, dict):
                    total = Decimal(str(data.get('walletBalance', 0)))
                    free = Decimal(str(data.get('availableToWithdraw', total)))
                    used = total - free
            ...
            try:
                ...
                raise ExchangeError(f"Failed to fetch balance: {e2}") from e2
